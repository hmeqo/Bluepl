...
import threading
import traceback
from typing import Optional, Callable, Iterable, Any, overload
...
class Task(object):
    ...
    def __init__(self, target: Optional[Callable] = None, *,
                 args: Iterable = (), kwargs: Optional[dict] = None):
        self.target = target
        self.args = args
...
class TaskRunner(object):
    ...
    def __init__(self, que=queue.Queue, thr=threading.Thread):
        self.que = que()
        self.thr = thr(target=self._mainloop, daemon=True)
        self.started = False
    ...
    def emit(self, func: Callable, args: Iterable = (), kwargs: Optional[dict] = None, *, default=None, handler=Task):
        """Put a Task to queue."""
        task = handler(func, args=args, kwargs=kwargs)
    ...
    @overload
    def emitter(self, *, default=None, handler=Task) -> Callable[[Callable], Callable[..., Task]]:
        ...

    @overload
    def emitter(self, func: Callable, *, default=None, handler=Task) -> Callable[..., Task]:
        ...

    def emitter(self, func=None, *, default=None, handler=Task) -> Any:
        """A decorator of `.run`."""
        if func is None:
