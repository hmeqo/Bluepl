...
class Field(object):
    ...
    def __init__(self, type: ExpectType, validator: _t.Optional[ExpectValidators] = None):
...
def main():
    from ahfakit.datamb.form import Form, Field, required, equals, lprint

    form = Form(
        username=Field(str, [
            required(on_invalid=lprint("不能为空")),
            equals("abc", on_invalid=lprint("账号错误")),
        ]),
